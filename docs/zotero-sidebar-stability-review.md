...
- 预期产物：`.scaffold/build/DS Copilot-0.1.0.xpi`
- 若构建失败，本轮直接不通过
...
- 如果 `.xpi` 导入后 Add-ons 列表里没有 `DS Copilot`，立即停止，先只排查启动和安装链路。
...
- 打开 DS Copilot 时原生右栏正文真的被隐藏，关闭时完整恢复
...
3. 在 Zotero 中安装 `.scaffold/build/DS Copilot-0.1.0.xpi`
4. 确认 Zotero 没有报出插件加载错误，且 Add-ons 列表中出现 `DS Copilot`

### 首次启动

1. 打开 Zotero 主界面
2. 打开 Zotero 设置并确认 `DS Copilot` pane 可正常打开
3. 观察右侧原生侧栏中是否出现 DS Copilot UI
4. 预期最少能看到稳定壳子：
   - 标题
   - scope 区域或空状态
   - 线程区占位
...
- `.xpi` 导入后 Add-ons 列表里没有 `DS Copilot`
